...
Telegram (англ. UI)
      │
      ▼
Бот на Railway (TypeScript)  ──── запрос свежего access_token ───►  n8n (облако клиента)
      │   сам ходит в Google API                                      хранит Google OAuth
      ├──────────────► Google Calendar API (events CRUD)              (Calendar+Gmail scopes),
      └──────────────► Gmail API (читает входящие → выжимка)          обновляет refresh_token
      │
      ▼
...
Заменяется: слой OAuth + google-client (вместо самописного refresh-token → токен от n8n),
хранилище (SQLite на Mac → Postgres на Railway), хостинг (LaunchAgent → Railway).

## Главные инварианты (полный список — domain-rules.yaml)

1. **product_language_english** — весь пользовательский текст на английском.
2. **n8n_is_token_keeper_only** — n8n только хранит/отдаёт Google-доступ, операции делает бот.
...
- **Google**: `@googleapis/calendar` + Gmail API через `googleapis`. Access-token от n8n.
